...
const API_URL = "https://script.google.com/macros/s/AKfycbzJNibMcKNs-Y5hTk1bHw3JAsqBxTgTRcXuV7j-c4NJzmFXgFmlP08Yr-QHMrR52teF/exec";
...
function propertyTelegramUrl(property) {
  const tg = (SITE_SETTINGS && SITE_SETTINGS.office_telegram) || FALLBACK_TELEGRAM;
  return tg;
}

function normalizePhone(phone) {
  return String(phone || "").replace(/\s+/g, "").replace(/^\+/, "").replace(/^0/, "966");
}
  ...
  return `
    <article class="card">
      ...
      <div class="card-actions">
        <a class="btn btn-secondary" href="property-details.html?id=${p.property_id}">التفاصيل</a>
        ...
        <a class="btn btn-secondary" target="_blank" href="${propertyTelegramUrl(p)}">تلغرام</a>
      </div>
...
async function loadHomeStats() {
  ...
  const approvedCount = (approved.success && approved.data) ? approved.data.length : 0;
  const pendingCount = (pending.success && pending.data) ? pending.data.length : 0;
...
async function loadFeaturedProperties() {
  ...
  const items = (res.success && res.data ? res.data : []).slice(0, 3);
  wrap.innerHTML = items.length ? items.map(x => cardTemplate(x)).join("") : `<div class="empty-state">لا توجد عقارات معتمدة حاليًا.</div>`;
}
...
async function initPropertiesPage() {
  ...
  function render() {
    ...
    grid.innerHTML = filtered.map(x => cardTemplate(x)).join("");
...
async function initPropertyDetailsPage() {
  ...
  if (!id) {
    wrap.innerHTML = `<div class="empty-state">لم يتم العثور على معرف العقار.</div>`;
    return;
  }
  ...
  wrap.innerHTML = `
    <div class="details-panel">
      ...
      <div class="info-grid">
        ...
        <div class="info-box"><span>المعلن</span><strong>${p.owner_name || "غير محدد"}</strong></div>
      </div>
    </div>
    <aside class="side-panel">
      ...
      <div class="inline-actions">
        ...
        <a class="btn btn-secondary full" target="_blank" href="${propertyTelegramUrl(p)}">تلغرام</a>
        <a class="btn btn-ghost full" target="_blank" href="${p.maps_url || '#'}">فتح Google Maps</a>
      </div>
      <div class="info-grid" style="margin-top:18px">
        <div class="info-box"><span>رقم العقار</span><strong>${p.property_id || "-"}</strong></div>
        <div class="info-box"><span>الحالة</span><strong>${statusText(p.status)}</strong></div>
      </div>
    </aside>
  `;
}
...
async function initRegisterPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (res.success) {
      ...
      showMessage("registerMessage", "تم إنشاء الحساب بنجاح. تم تسجيل دخولك تلقائيًا.");
...
async function initAddPropertyPage() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (res.success) {
      form.reset();
      showMessage("propertyMessage", "تم استلام العقار بنجاح، وسيتم نشره بعد مراجعة المكتب.");
    } else {
...
async function initMyPropertiesPage() {
  ...
  const items = (res.success ? res.data : []).filter(p => String(p.owner_id) === String(user.id));
  grid.innerHTML = items.map(x => cardTemplate(x)).join("");
...
async function loadAdminProperties() {
  ...
  const approvedData = approved.success ? approved.data : [];
  const rejectedData = rejected.success ? rejected.data : [];
...
async function approveProperty(id) {
  ...
  if (res.success) await loadAdminProperties();
  else alert(res.message || "تعذر تنفيذ العملية");
}
...
async function toggleAdminRole(userId, currentRole) {
  ...
  else alert(res.message || "تعذر تعديل الصلاحية");
}
...
async function removeUser(userId) {
  ...
  if (res.success) await loadAdminUsers();
  else alert(res.message || "تعذر حذف المستخدم");
}
